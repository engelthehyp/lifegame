...
function getInfo(e) {
	...
	lifeArray.forEach(life => life.selected = life.x === x && life.y === y);
}
...
function update(e) {
	...
	updateStatGui();
	if (++totalTurnCount, ++turnCount >= 50) createFood(50 + (turnCount = 0)); // Channeling C
}
...
function createFood(number) {
	const dimension = d => Math.floor((Math.random() * d) / 10);
	let count = 0;

	while (count <= number) {
		let x = dimension(canvas.width);
		let y = dimension(canvas.height);
		//let x = 11;
		//let y = 8;
